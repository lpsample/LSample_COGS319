...
bots <- createGeneration()
generation.results <- runGeneration(bots, 2)

next.gen<- function(old.gen){
next.generation <- sample(old.gen$bot.number, 10, replace=T, prob = generation.results$fitness)
#next.generation <- c(1,1,1,1,4,5,6,3,3,4)
next.bots.genomes <- bots[next.generation]
...
mutate <- function(weights){
  ...
  dim(weights)
  r<- sample(nrow(weights), 1)
  c<- sample(ncol(weights), 1)
  new.weights[r][c] <- sample(weight.options, 1)
  return(new.weights)
  }
...
run<- function(x){
  genome.0<- createGeneration()
  
  gen.0<- runGeneration(genome.0, x)
  genome.1<- next.gen(gen.0)
  genome.1.m <- mutate(genome.1)
  gen.1<- runGeneration(genome.1.m, x)
