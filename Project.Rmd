...
getlight<- function(obsGrid, lightGrid){
  aCur<- which(obsGrid==0, arr.ind=TRUE)
  #up
  if(all(cardinal)== up){
...
options<- list(moveForward(obsGrid), moveBackward(obsGrid), turnClock(obsGrid), turnCounter(obsGrid))
rand<-sample(1:4, 1, replace = TRUE)
  move<- options[rand]
  obsGrid<- move
